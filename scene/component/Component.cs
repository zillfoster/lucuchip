...
public partial class Component : Node2D, IMouseInputable
{
    public Rect2I Field
    {
        get => _field;
    ...
    public ComponentUnit ChosenDrawStyle { get; set; } = ComponentUnit.None;
    ...
    {
        if (IsEditable && FieldContains(position))
            _mainLayer.AssignUnit(CoordsFrom(position), ComponentUnit.Erase);
    }
    public void ClearUnit() { if (IsEditable) _mainLayer.Clear(); }
    public ComponentUnit GetUnit(Vector2 position)
        => _mainLayer.GetUnit(CoordsFrom(position));
    public Dictionary<Vector2I, ComponentUnit> GetUnits()
    {
        Dictionary<Vector2I, ComponentUnit> units = new();
        foreach (Vector2I coords in _mainLayer.GetUsedCells())
    ...
    [Export]
    private ComponentBackgroundLayer _backgroundLayer;
    [Export]
    private ComponentMainLayer _mainLayer;
    [Export]
    private ComponentCursorLayer _cursorLayer;
    ...
    void IMouseInputable.OnMouseButton(Vector2 position, MouseButton button, bool isPressed)
    {
        ...
        ComponentUnit style = isPressed? GetCurrentStyle(button): ChosenDrawStyle;
    ...
    {
        Vector2I coords = CoordsFrom(position);
        ComponentUnit style = GetCurrentStyle(lastButton);
        if (FieldContains(coords))
        {
            if (IsEditable &&
    ...
    private ComponentUnit GetCurrentStyle(MouseButton button)
        => button == MouseButton.Right? ComponentUnit.Erase: ChosenDrawStyle;
    ...
    private void DrawThrough(Vector2 position, Vector2 relative, ComponentUnit style)
    {
